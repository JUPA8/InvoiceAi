...
import { Cost_card } from "./cost_card";
...
export function Cost_Centers() {
    ...
    const getClientId = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.warn("Session endpoint failed.");
  ...
  const loadCostCenters = React.useCallback(async () => {
    ...
    try {
      ...
      const mappedData: Payment[] = costCenters.map((center) => ({
        id: center.id,
        name: center.name,
        url: center.applicationUrl || "",
        status: center.isActive ? "Active" : "Inactive",
        created_at: center.createdDate
          ? new Date(center.createdDate).toISOString().split("T")[0]
          : "",
      ...
      setError(error.message || "Failed to load cost centers.");
    } finally {
      setLoading(false);
    }
  }, [clientId]);
  ...
  const handleDelete = React.useCallback(
    async (item: Payment) => {
      ...
      try {
        setError(null);
        await deleteCostCenter(clientId, item.id);

        await loadCostCenters();
      } catch (error: any) {
        setError(error.message || "Failed to delete cost center.");
      }
    },
    [clientId, tableData, loadCostCenters]
  );
  ...
  const stats = React.useMemo(
    () => ({
      ...
      active: tableData.filter((item) => item.status === "Active").length,
      inactive: tableData.filter((item) => item.status === "Inactive").length,
    }),
    [tableData]
  );
          ...
          <>
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((item) => (
                    ...
                    >
                      ...
                      <TableCell className="py-2">
                        <div className="text-gray-700">{item.created_at}</div>
                      </TableCell>
                      <TableCell className="py-2">
                        <DropdownMenu>
                          ...
                          >
                            <DropdownMenuItem
                              onClick={() => {
                                setModalInfo({ isOpen: true, item: item });
                              }}
            ...
            <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 flex items-center justify-between">
              <div className="text-sm font-medium text-gray-500">
                Showing {paginatedData.length} of {filteredData.length} cost
                centers
              </div>
